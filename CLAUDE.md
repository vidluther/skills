...
Skills may be forks of public skill sets vendored into `skills/` and then adapted (currently six, all from mattpocock/skills: `diagnose`, `tdd`, `improve-codebase-architecture`, `rubber-duck`, `to-issues`, `to-prd`). When vendored, they are **ours now** — we never auto-update them.
